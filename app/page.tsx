...
import { SCRIPTS, Script, ScriptSection } from '@/data/scripts'
...
function resolveVars(text: string, lead: Opportunity | null): string {
  ...
  const map: Record<string,string> = {
    ...
    '{Fecha de Registro}': regDate, '{Tu Número}': '+1 (XXX) XXX-XXXX',
  }
...
const C = {
  ...
  text:'#1a1e3a', textMuted:'#6B7280', textLight:'#9CA3AF',
}

const outcomeColor = (color: string) =>
  ({ green:'#16a34a', yellow:'#d97706', red:C.red, blue:C.blue }[color] || C.textMuted)
...
export default function Dashboard() {
  ...
  // ── fetch (silent = no loading spinner, for auto-refresh) ──────────────
  const fetchLeads = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // Refresh selectedLead data if still present
      setSelectedLead(prev => {
        if (!prev) return prev
        return newOpps.find(o => o.id === prev.id) ?? prev
      })
  ...
  const confirmMove  = async () => {
    ...
    try {
      ...
      setOpportunities(prev => prev.map(o =>
        o.id === selectedLead.id ? { ...o, pipelineStageId: moveTargetStage.id } : o
      ))
  ...
  return (
    <div style={{display:'flex',height:'100vh',background:C.bg,color:C.text,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",overflow:'hidden'}}>
      ...
      {/* Toast */}
      {toast && <div style={{position:'fixed',top:16,right:16,zIndex:9999,background:toast.type==='success'?'#16a34a':C.red,color:'#fff',padding:'10px 18px',borderRadius:8,fontSize:13,fontWeight:600,boxShadow:'0 4px 20px rgba(0,0,0,0.2)',animation:'fadeIn 0.2s ease'}}>{toast.msg}</div>}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(15,20,91,0.45)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(3px)'}}>
          <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:16,padding:28,width:380,boxShadow:'0 24px 60px rgba(15,20,91,0.25)'}}>
            ...
            <p style={{fontSize:13,color:C.textMuted,marginBottom:4}}>
              Vas a mover a <strong style={{color:C.darkBlue}}>{getFullName(selectedLead)}</strong> de:
            </p>
            <div style={{background:C.bg,borderRadius:10,padding:'10px 14px',marginBottom:16,fontSize:13}}>
              <span style={{color:C.textMuted}}>Desde: </span>
              <span style={{fontWeight:600,color:C.darkBlue}}>{selectedStage?.name}</span>
            ...
            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>{setShowMoveModal(false);setMoveTargetStage(null)}}
                style={{flex:1,padding:'10px 0',background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,color:C.textMuted,cursor:'pointer',fontSize:13,fontFamily:'inherit',fontWeight:600}}>
                Cancelar
              </button>
              <button onClick={confirmMove} disabled={moving}
                style={{flex:1,padding:'10px 0',background:moving?'#6B7280':C.blue,border:'none',borderRadius:10,color:'#fff',cursor:moving?'not-allowed':'pointer',fontSize:13,fontFamily:'inherit',fontWeight:700}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:C.bg}}>
          ...
          <div style={{padding:'12px 20px',borderBottom:`1px solid ${C.border}`,display:'flex',alignItems:'center',gap:12,background:C.white,boxShadow:'0 1px 4px rgba(40,58,151,0.06)'}}>
            ...
            <div style={{position:'relative',marginRight:8}}>
              <select
                onChange={e=>{
                  const t = stages.find(s=>s.id===e.target.value)
                  if (t && t.id !== selectedStage?.id) initiateMove(t)
                  e.target.value = ''
                }}
                defaultValue=""
              ...
              >
                ...
                {stages.filter(s=>s.id!==selectedStage?.id).map(s=>(
