...
type Opportunity = {
  ...
  contact?: {
    ...
    customFields?: Array<{ id: string; value: string; fieldKey?: string; name?: string }>
  }
...
function resolveVars(text: string, lead: Opportunity | null): string {
  ...
  const apptRaw = c?.appointmentDate ?? ''
  let apptDate = '', apptTime = ''
  if (apptRaw) {
    const d = new Date(apptRaw)
    if (!isNaN(d.getTime())) {
      apptDate = d.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })
      apptTime = d.toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })
    }
  }
...
export default function Dashboard() {
    ...
    return (
      <div key={i} style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 700, color: titleColor, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            {s.title}
          </span>
        </div>
        <div style={{
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: C.bg, color: C.text, fontFamily: "'Inter','Segoe UI',system-ui,sans-serif", overflow: 'hidden', position: 'relative' }}>
      ...
      {showNotAvailable && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,20,91,0.4)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(2px)' }}>
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 16, padding: 28, width: 340, boxShadow: '0 20px 60px rgba(15,20,91,0.2)' }}>
            ...
            <input type="number" min="1" max="72" value={notAvailableHours} onChange={e => setNotAvailableHours(e.target.value)} placeholder="ej: 2" style={{ width: '100%', background: C.bg, border: `1px solid ${C.border}`, borderRadius: 8, padding: '8px 12px', color: C.text, fontSize: 14, marginBottom: 16, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' as const }} />
      ...
      <div style={{ width: 224, background: C.sidebar, display: 'flex', flexDirection: 'column', flexShrink: 0, boxShadow: '2px 0 12px rgba(15,20,91,0.2)' }}>
        <div style={{ padding: '18px 16px 14px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 34, height: 34, background: C.red, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>💬</div>
            <div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px 8px' }}>
          {loading
            ...
            : stages.length === 0
              ? <div style={{ padding: 16, fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>Sin stages. Verifica Pipeline ID.</div>
              : stages.map(stage => {
                ...
                return (
                  <button key={stage.id} onClick={() => { setSelectedStage(stage); setSelectedLead(null) }}
                    style={{ width: '100%', textAlign: 'left', background: isActive ? C.sidebarActive : 'transparent', border: 'none', borderRadius: 8, padding: '8px 10px', cursor: 'pointer', marginBottom: 2, color: isActive ? '#fff' : 'rgba(255,255,255,0.55)', fontSize: 11.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'all 0.15s', fontFamily: 'inherit', fontWeight: isActive ? 600 : 400 }}>
                    <span style={{ lineHeight: 1.4 }}>{stage.name}</span>
                    {count > 0 && <span style={{ background: isActive ? C.red : 'rgba(255,255,255,0.12)', color: '#fff', borderRadius: 10, padding: '1px 7px', fontSize: 10, fontWeight: 700, flexShrink: 0, marginLeft: 4 }}>{count}</span>}
                  </button>
                )
              })}
        </div>
        <div style={{ padding: '10px 8px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <button onClick={fetchLeads} style={{ width: '100%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: '7px 0', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: 11, fontFamily: 'inherit', fontWeight: 500 }}>
            ↻ Actualizar
          </button>
        </div>
      </div>
      ...
      <div style={{ width: 264, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', flexShrink: 0, background: C.white }}>
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          {leadsInStage.length === 0
            ? <div style={{ padding: 24, fontSize: 12, color: C.textLight, textAlign: 'center' as const }}>{selectedStage ? 'Sin leads en este paso' : 'Selecciona un paso del pipeline'}</div>
            : leadsInStage.map(opp => {
              ...
              return (
                <div key={opp.id} onClick={() => { setSelectedLead(opp); setActiveTab('script'); setShowVoicemail(false); setShowNotAnswered(false) }}
                  style={{ padding: '10px 12px', cursor: 'pointer', borderBottom: `1px solid ${C.border}`, background: isSelected ? '#EEF2FF' : 'transparent', borderLeft: isSelected ? `3px solid ${C.blue}` : '3px solid transparent', transition: 'all 0.1s' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: 10.5, color: C.textLight, marginTop: 1 }}>{opp.contact?.phone ?? opp.source ?? '—'}</p>
                    </div>
                  </div>
                </div>
              )
      ...
      {selectedLead ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: C.bg }}>
          ...
          <div style={{ padding: '12px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 12, background: C.white, boxShadow: '0 1px 4px rgba(40,58,151,0.06)' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: 11, color: C.textMuted, marginTop: 1 }}>
                {selectedLead.contact?.email ?? '—'} · {selectedLead.contact?.phone ?? '—'}{selectedLead.contact?.timezone ? ` · ${selectedLead.contact.timezone}` : ''}
              </p>
            </div>
            <a href={`https://app.funnelup.io/v2/location/${process.env.NEXT_PUBLIC_GHL_LOCATION_ID}/contacts/detail/${selectedLead.contactId}`} target="_blank" rel="noopener noreferrer"
          ...
          <div style={{ display: 'flex', borderBottom: `1px solid ${C.border}`, padding: '0 20px', background: C.white }}>
            {(['script', 'activity', 'info'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                style={{ padding: '11px 16px', background: 'transparent', border: 'none', borderBottom: activeTab === tab ? `2px solid ${C.red}` : '2px solid transparent', color: activeTab === tab ? C.red : C.textMuted, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', letterSpacing: '0.04em', textTransform: 'uppercase' as const, marginBottom: '-1px', transition: 'color 0.15s' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
                ...
                {currentScript ? (
                  <>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20, padding: '10px 14px', background: C.white, borderRadius: 10, border: `1px solid ${C.border}`, boxShadow: '0 1px 4px rgba(40,58,151,0.05)' }}>
                      <span style={{ background: C.darkBlue, color: '#fff', borderRadius: 6, padding: '3px 10px', fontSize: 10.5, fontWeight: 700, letterSpacing: '0.04em' }}>{currentScript.stepName}</span>
                      ...
                      <button onClick={() => setShowVoicemail(!showVoicemail)}
                        style={{ marginLeft: 'auto', background: showVoicemail ? C.blue : C.bg, border: `1px solid ${showVoicemail ? C.blue : C.border}`, borderRadius: 8, padding: '5px 12px', color: showVoicemail ? '#fff' : C.blue, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, transition: 'all 0.15s' }}>
                    ...
                    {!showVoicemail && currentScript.answeredOutcomes.length > 0 && (
                      <div style={{ marginTop: 24, background: C.white, borderRadius: 12, border: `1px solid ${C.border}`, padding: '16px 18px', boxShadow: '0 1px 4px rgba(40,58,151,0.06)' }}>
                        {!showNotAnswered ? (
                          <>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, marginBottom: 10 }}>
                              {currentScript.answeredOutcomes.map((o, i) => (
                                <button key={i} onClick={() => handleOutcome(o.action, o.label)}
                                  style={{ padding: '10px 12px', background: `${outcomeColor(o.color)}15`, border: `1.5px solid ${outcomeColor(o.color)}`, borderRadius: 8, color: outcomeColor(o.color), fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700, transition: 'all 0.15s' }}>
                                  {o.label}
                                </button>
                              ))}
                            </div>
                            <button onClick={() => setShowNotAnswered(true)}
                              style={{ width: '100%', padding: '9px 0', background: C.bg, border: `1px solid ${C.border}`, borderRadius: 8, color: C.textMuted, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500 }}>
                              No contestó →
                            </button>
                          </>
                        ) : (
                  ...
                  <div style={{ padding: 32, textAlign: 'center' as const, color: C.textLight, fontSize: 13, background: C.white, borderRadius: 12, border: `1px solid ${C.border}` }}>
                    ...
                    <span style={{ fontSize: 11, color: C.textLight, marginTop: 6, display: 'block' }}>Stage: {selectedStage?.name}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'activity' && (
              <div>
                <div style={{ marginBottom: 16, background: C.white, borderRadius: 12, border: `1px solid ${C.border}`, padding: '14px 16px', boxShadow: '0 1px 4px rgba(40,58,151,0.05)' }}>
                ...
                {notes.length === 0
                  ...
                  : notes.map(note => (
                    <div key={note.id} style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px', marginBottom: 8, boxShadow: '0 1px 3px rgba(40,58,151,0.05)' }}>
            ...
            {activeTab === 'info' && (
              <div style={{ background: C.white, borderRadius: 12, border: `1px solid ${C.border}`, overflow: 'hidden', boxShadow: '0 1px 4px rgba(40,58,151,0.05)' }}>
                {[
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 14, background: C.bg }}>
          <div style={{ width: 72, height: 72, borderRadius: '50%', background: C.white, border: `2px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, boxShadow: '0 4px 16px rgba(40,58,151,0.08)' }}>📞</div>
